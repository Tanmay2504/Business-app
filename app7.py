        ...
        with tab3:
            ...
            if not st.session_state.sales_history.empty:
                ...
                fig.add_trace(go.Scatter(
                    x=st.session_state.sales_history['Date'],
                    y=st.session_state.sales_history['Sales Quantity'],
            ...
            if not st.session_state.customers.empty:
                ...
                with col2:
                    if not st.session_state.sales_history.empty:
                        ...
                        fig = px.bar(
                            customer_sales.sort_values('Net Amount', ascending=False).head(10),
                            x='Customer Name',
                            y='Net Amount',
            ...
            if not st.session_state.suppliers.empty:
                ...
                with col1:
                    fig = px.histogram(
                        ...
                        x='Rating',
                        title='Supplier Ratings Distribution'
                    )
                    st.plotly_chart(fig)
